 ...
 * Uses Firebase Google authentication when the real SDK and provider are
 * configured. It never creates a local stand-in account.
 */
(function (root) {
  "use strict";
  ...
  function isReady() {
    return Boolean(root.firebase && root.firebase.auth);
  }
  ...
  async function signInWithGoogle() {
    if (isReady()) {
      try {
        ...
        // Authentication alone never grants a team role. Server-backed team
        // membership must be configured and verified before authorization.
        return user;
      } catch (err) {
    ...
    console.warn("Secure sign-in is not configured. No local account was created.");
    return null;
  }
  ...
  function updateAuthUi() {
    ...
    authContainers.forEach(function (container) {
      if (user) {
        ...
        if (!isReady()) {
          container.innerHTML = '<p class="tiny" role="status">Secure sign-in setup is pending Firebase owner access.</p>';
          return;
        }
        container.innerHTML = [
