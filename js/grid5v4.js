...
(function () {
  ...
  function lerpPoses(tval) {
    ...
    return { p, runner: A.runner, lead: A.lead, hole: A.hole, ball: { x: p[A.runner].x, y: p[A.runner].y } };
  }
  ...
  function applyPoses() {
    const { p, runner, lead, hole, ball } = (Math.abs(state.t - Math.round(state.t)) < 0.001) ? poses(state.beat) : lerpPoses(state.t);
    Object.keys(p).forEach((id) => {
      ...
      if (node.role) node.role.textContent = id === runner ? "BALL" : (id === lead ? "LEAD" : "");
    });
    ...
    if (beatEl) beatEl.textContent = b.name;
    if (slider) slider.value = String(state.t);
    if (state.trail) {
      const pt = el("circle", { cx: String(ball.x), cy: String(ball.y), r: "3.5", fill: "#e8b423", opacity: "0.45" });
      state.trail.appendChild(pt);
      while (state.trail.childNodes.length > 28) state.trail.removeChild(state.trail.firstChild);
    }
  ...
  function token(id, spec) {
    ...
    const role = el("text", { x: "0", y: "40", fill: "#e8b423", "font-size": "11", "font-weight": "800", "text-anchor": "middle", "font-family": "system-ui,sans-serif", "pointer-events": "none" });
    g.appendChild(hit);
    g.appendChild(ring);
    g.appendChild(disc);
    g.appendChild(lab);
    ...
    svg.appendChild(g);
    state.players[id] = { id, g, disc, ring, role, x: spec.x, y: spec.y };
  }
